...
//  uint32_t x_step  = 0x000101; // sparse test
    ...
    for (uint32_t x = x_start; x <= x_stop; x += x_step)
    {
        ...
        atPosEdgeClk(top, contextp, trace_fp);
        clock_cntr++;
        y_rtl_err = checkOutput(top, lat_fifo);
        if (x == x_start || y_rtl_err > max_y_rtl_err)
        {
          max_y_rtl_err = y_rtl_err;
        }
    }
    ...
    cout << endl << "MAX RTL ERR: " << max_y_rtl_err << endl;
    
    if (max_y_rtl_err < ERR_TOL)
    {
      cout << "TOLERANCE ACCEPTABLE!" << endl;
    }
    else
    {
      cout << "ERROR: MAX ERROR IS TOO BIG!!!" << endl;
    }
    
    cout << "TOLERANCE: " << ERR_TOL << endl;
